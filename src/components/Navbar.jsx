...
const Navbar = () => {
  ...
  const isAtTop = isHome && scrollY < 50;
    ...
    <nav className={`fixed top-0 w-full z-[100] px-6 lg:px-12 py-6 transition-all duration-700 ${!isAtTop ? 'bg-surface/90 backdrop-blur-xl py-4 shadow-2xl border-b border-black/5' : ''} ${isAtTop ? 'text-white' : 'text-text-primary'}`}>
      <div className="max-w-[1800px] mx-auto flex justify-between items-end">
        <Link to="/" className="flex group cursor-pointer">
          <img src={logo} alt="Brisk Logo" className={`h-10 lg:h-12 w-auto object-contain ${isAtTop ? 'brightness-0 invert' : ''}`} />
        </Link>

        <div className="hidden lg:flex items-center gap-6 font-body text-[11px] uppercase tracking-[0.4em] font-extrabold">
          {navLinks.map((link) => (
            <NavLink
              key={link.name}
              to={link.href}
          ...
          <Link to="/contact" className={`flex items-center gap-3 px-6 py-2.5 rounded-none font-extrabold transition-all duration-300 group ml-2 border ${isAtTop
              ...
              {({ isActive }) => (
                <>
                  <div className="relative group">
                    <span className="text-2xl font-extrabold tracking-tighter uppercase font-heading">{link.name}</span>
