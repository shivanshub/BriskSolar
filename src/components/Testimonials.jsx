        ...
        <div className="grid lg:grid-cols-2 gap-12">
          {reviews.map((rev, i) => (
            <div key={i} className="bg-surface border border-black/5 p-12 rounded-sm space-y-8 hover:bg-primary/5 transition-colors duration-700 relative overflow-hidden group">
               ...
               <p className="text-xl font-bold leading-relaxed text-text-primary relative z-10">
                  "{rev.quote}"
               </p>
               <div className="space-y-1">
                  <h4 className="text-text-primary font-body font-extrabold text-sm uppercase tracking-widest">{rev.name}</h4>
                  <p className="text-text-muted font-body text-[10px] uppercase tracking-[0.2em]">{rev.city}, Punjab</p>
               </div>
            </div>
          ))}
        </div>
