            ...
            {points.map((point, i) => (
              <div key={i} className="why-point flex flex-col lg:flex-row gap-8 p-10 border border-black/5 bg-surface/50 transition-all duration-700 rounded-sm">
                ...
                <div className="space-y-4">
                  <h3 className="text-2xl font-extrabold tracking-tighter text-text-primary font-heading">{point.title}</h3>
                  <p className="font-body text-sm text-text-muted leading-relaxed max-w-2xl">{point.desc}</p>
                </div>
              </div>
            ))}
          </div>
